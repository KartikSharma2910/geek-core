...
export function VPN() {
  ...
  return (
    <div className="p-4 space-y-4 text-xs">
      {/* Status hero */}
      <div
        className={`neon-border rounded-lg p-4 ${connected ? "bg-[var(--neon)]/10" : "bg-black/40"}`}
      >
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-3">
            {connected ? (
              <Shield className="h-10 w-10 text-[var(--neon)] pulse-glow" />
            ) : (
              <ShieldOff className="h-10 w-10 opacity-60" />
            )}
            <div>
              ...
              <div className="opacity-70">
                {connected
                  ? `via ${node.flag} ${node.city} · ${proto}`
                  : "Direct connection — your real IP is exposed"}
              </div>
            </div>
          </div>
          <button
            ...
            className={`px-5 py-2 rounded-md neon-border font-bold tracking-wider text-xs ${
              connected
                ...
                : "bg-[var(--neon)]/20 hover:bg-[var(--neon)]/40"
            }`}
          >
            {connecting ? (
      ...
      <div className="neon-border rounded-lg p-3">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2 neon-text">
            <Globe2 className="h-3.5 w-3.5" /> IP Intelligence
          </div>
          <button
            onClick={loadIp}
            className="text-[10px] px-2 py-0.5 neon-border rounded hover:bg-[var(--neon)]/15"
          >
            Refresh
          </button>
        </div>
        {loading ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
            ...
            <Field
              ...
              label="ISP / Org"
              value={connected ? "GeekVPN Network" : (info?.org ?? "—")}
            />
      ...
      {/* Exit nodes */}
      <div className="neon-border rounded-lg p-3">
        <div className="flex items-center gap-2 mb-2 neon-text">
          <Wifi className="h-3.5 w-3.5" /> Exit Nodes
        </div>
      ...
      <div className="neon-border rounded-lg p-3 space-y-2">
        ...
        <Toggle
          label="Kill switch (block traffic if VPN drops)"
          value={killSwitch}
        ...
        <div className="text-[10px] opacity-50 pt-1">
          Open-source style WireGuard/OpenVPN front-end. This build simulates the tunnel for
          educational purposes — your real traffic is not routed.
        </div>
      </div>
    </div>
  );
