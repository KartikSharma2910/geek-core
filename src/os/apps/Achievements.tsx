    ...
    <div className="p-4 space-y-3 text-sm">
      <div className="flex items-center justify-between neon-border rounded p-3 bg-black/30">
        <div>
          ...
          <div className="neon-text text-xl">{unlocked.size} / {ACHIEVEMENTS.length}</div>
        </div>
        <div>
      ...
      <div className="h-1.5 bg-[var(--neon)]/15 rounded overflow-hidden">
        <div className="h-full bg-[var(--neon)]" style={{ width: `${pct}%`, boxShadow: "0 0 10px var(--neon)" }} />
      </div>
      <div className="grid sm:grid-cols-2 gap-2">
        {ACHIEVEMENTS.map((a) => {
          const got = unlocked.has(a.id);
          return (
            <div key={a.id} className={`neon-border rounded p-3 ${got ? "bg-[var(--neon)]/10" : "bg-black/30 opacity-60"}`}>
              <div className="flex justify-between items-start">
                <div className={got ? "neon-text" : ""}>{got ? "🏆 " : "🔒 "}{a.title}</div>
