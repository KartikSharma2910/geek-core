  ...
  return (
    <div>
      <div className="flex justify-between text-xs mb-1 opacity-80">
        <span>{label}</span>
        <span className="neon-text">{value.toFixed(1)}{unit}</span>
      </div>
      <div className="h-2 bg-[var(--neon)]/15 rounded">
        <div className="h-full rounded bg-[var(--neon)] transition-all" style={{ width: `${pct}%`, boxShadow: "0 0 10px var(--neon)" }} />
      </div>
    </div>
  );
}
...
export function SystemMonitor() {
  ...
  const [stats, setStats] = useState<Stats>({
    cores: "—", mem: "—", online: true, conn: "unknown", downlink: "—", battery: null,
    platform: "—", ua: "—",
  });
  ...
  useEffect(() => {
    const nav = navigator as Navigator & {
      deviceMemory?: number;
      connection?: { effectiveType?: string; downlink?: number; addEventListener?: (e: string, f: () => void) => void };
    ...
    nav.getBattery?.().then((b) => {
      const upd = () => setStats((s) => ({ ...s, battery: { level: b.level, charging: b.charging } }));
      upd();
    ...
    <div className="p-4 space-y-4 text-sm">
      <div>
        ...
        <svg viewBox="0 0 400 80" className="w-full h-24 neon-border rounded bg-black/40">
          <polyline
            fill="none" stroke="var(--neon)" strokeWidth="1.5"
            points={cpu.map((v, i) => `${(i / (cpu.length - 1)) * 400},${80 - (v / 100) * 75}`).join(" ")}
            style={{ filter: "drop-shadow(0 0 4px var(--neon))" }}
          />
        </svg>
        <div className="text-[10px] opacity-60 mt-1">avg fps ≈ {fpsHist.length ? Math.round(fpsHist.reduce((a, b) => a + b, 0) / fpsHist.length) : 0}</div>
      </div>

      {typeof stats.mem === "number" && <Bar label={`Memory (~${stats.mem} GB device)`} value={memEst} />}
      {stats.battery && (
        <Bar label={`Battery — ${stats.battery.charging ? "charging ⚡" : "on battery"}`} value={battPct} />
      )}
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3 pt-2 text-[10px]">
        {[
          ...
          ["Downlink", typeof stats.downlink === "number" ? `${stats.downlink} Mbps` : stats.downlink],
