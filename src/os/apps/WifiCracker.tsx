...
const ESSIDS = [
  "HomeNet_5G", "CoffeeShop_Free", "FBI_Surveillance_Van",
  "NETGEAR_42", "DIRECT-roku-882", "xfinitywifi",
  "Pretty_Fly_4_WiFi", "TellMyWiFiLoveHer", "VM-3F8A1",
  "iotcam-7c", "TP-Link_2C4F", "starbucks-guest",
  "linksys", "AndroidAP_9921", "neonlab-internal",
];
const WORDLIST = [
  "12345678", "password", "qwerty123", "letmein!", "admin123",
  "summer2024", "iloveyou", "monkey99", "dragon77", "ninja2k",
  "neon4ever", "operator", "hacktheplanet", "trustno1", "cyber2025",
];
...
function genNetworks(n = 12): Net[] {
  ...
  return Array.from({ length: n }, () => {
    ...
    while (used.has(essid)) essid = ESSIDS[Math.floor(Math.random() * ESSIDS.length)] + "_" + Math.floor(Math.random() * 99);
    used.add(essid);
    const enc: Net["enc"] = (["WPA2", "WPA2", "WPA2", "WPA3", "WEP", "OPEN"] as const)[Math.floor(Math.random() * 6)];
    return {
      bssid: randBSSID(),
      essid,
...
export function WifiCracker() {
  ...
  const [logs, setLogs] = useState<string[]>(["airmon-ng start wlan0", "monitor mode enabled on wlan0mon"]);
  ...
  useEffect(() => { logRef.current?.scrollTo({ top: 9e9 }); }, [logs]);
    ...
    const t = setInterval(() => {
      setNets((prev) => {
        const next = prev.map((n) => ({
          ...
          clients: Math.max(0, n.clients + (Math.random() < 0.1 ? (Math.random() < 0.5 ? -1 : 1) : 0)),
        }));
  ...
  const captureHandshake = async () => {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      log(`  [${new Date().toLocaleTimeString()}] beacon ${1 + Math.floor(Math.random() * 200)} CH:${target.channel} PWR:${target.signal}`);
    }
  ...
  const crack = async () => {
    if (!target || !target.handshake) { log("! capture a handshake first"); return; }
    setCracking(true); stopRef.current = false; setProgress(0); setTries(0);
    ...
    while (attempted < total) {
      if (stopRef.current) { log("! aborted by operator"); break; }
      ...
      if (attempted >= hitAt && found) {
        ...
        log(`>> KEY FOUND! [ ${key} ]`);
        log(`time: ${((performance.now() - startedAt) / 1000).toFixed(1)}s · keys/s ≈ ${Math.round(attempted / ((performance.now() - startedAt) / 1000))}`);
    ...
    <div className="grid grid-cols-[1fr_320px] h-full text-xs">
      <section className="flex flex-col border-r border-[var(--neon)]/20">
        <div className="flex items-center gap-2 p-2 border-b border-[var(--neon)]/20 bg-black/30">
          <button
            onClick={() => { setScanning((s) => !s); if (!scanning) log("airodump-ng wlan0mon"); else log("scan stopped"); }}
            className="px-2 py-1 neon-border rounded hover:bg-[var(--neon)]/15 flex items-center gap-1 neon-text"
          >
            {scanning ? <><Square className="h-3 w-3" /> STOP</> : <><Play className="h-3 w-3" /> SCAN</>}
          </button>
          <span className="opacity-60">iface: wlan0mon · band: 2.4GHz · {nets.length} APs</span>
          {scanning && <span className="ml-auto h-2 w-2 rounded-full bg-[var(--neon)] pulse-glow" />}
        </div>
          ...
          <table className="w-full text-[11px]">
            <thead className="sticky top-0 bg-black/70">
              <tr className="text-[10px] uppercase opacity-60">
                ...
                <th>CH</th><th>PWR</th><th>ENC</th><th>CL</th><th>HS</th>
              </tr>
            </thead>
            <tbody>
              {nets.length === 0 && (
                <tr><td colSpan={7} className="p-6 text-center opacity-50">
                  {scanning ? "scanning..." : "press SCAN to begin"}
                </td></tr>
              )}
              {nets.sort((a, b) => b.signal - a.signal).map((n) => {
                const bars = n.signal > -55 ? "▮▮▮▮" : n.signal > -70 ? "▮▮▮" : n.signal > -82 ? "▮▮" : "▮";
                ...
                return (
                  ...
                  >
                    ...
                    <td>{n.essid} {n.cracked && <Unlock className="inline h-3 w-3 ml-1 text-[var(--neon)]" />}</td>
                    <td className="text-center">{n.channel}</td>
                    <td className="text-center"><span className="neon-text">{bars}</span> <span className="opacity-50 text-[10px]">{n.signal}</span></td>
                    <td className="text-center">{n.enc === "OPEN" ? <span className="opacity-50">OPEN</span> : <span className="flex items-center justify-center gap-1"><Lock className="h-2.5 w-2.5" />{n.enc}</span>}</td>
                    <td className="text-center">{n.clients}</td>
                    <td className="text-center">{n.handshake ? <span className="neon-text">✓</span> : "—"}</td>
                  </tr>
                );
              })}
            </tbody>
      ...
      <aside className="flex flex-col bg-black/40">
        <div className="p-3 border-b border-[var(--neon)]/20">
          {!target ? (
            ...
            <>
              <div className="flex items-center gap-2 mb-2"><Wifi className="h-4 w-4 text-[var(--neon)]" /><span className="neon-text">{target.essid}</span></div>
              <div className="grid grid-cols-2 gap-1 text-[10px] opacity-80">
                <div>BSSID</div><div className="font-mono">{target.bssid}</div>
                <div>Channel</div><div>{target.channel}</div>
                <div>Signal</div><div>{target.signal} dBm</div>
                <div>Encryption</div><div>{target.enc}</div>
                <div>Clients</div><div>{target.clients}</div>
                <div>Handshake</div><div>{target.handshake ? "captured" : "—"}</div>
                {target.cracked && (<><div>Key</div><div className="neon-text">{target.cracked}</div></>)}
              </div>
              ...
              {cracking && (
                <div className="mt-2">
                  <div className="flex justify-between text-[10px] opacity-70">
                    <span>{tries} keys</span><span>{progress}%</span>
                  </div>
                  <div className="h-1 bg-[var(--neon)]/15 rounded mt-1">
                    <div className="h-full bg-[var(--neon)] rounded" style={{ width: `${progress}%`, boxShadow: "0 0 6px var(--neon)" }} />
                  </div>
                  <button onClick={() => { stopRef.current = true; }} className="w-full mt-1 text-[10px] opacity-60 hover:opacity-100">abort</button>
                </div>
              )}
            </>
          )}
        </div>
        <div ref={logRef} className="flex-1 overflow-auto p-2 font-mono text-[10px] leading-relaxed">
