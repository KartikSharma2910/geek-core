...
import {
  TerminalSquare, Activity, KeyRound, Network, Globe,
  FlaskConical, GraduationCap, Gamepad2, Settings as SettingsIcon, Info, Power, Search, Wifi, Trophy, Shield, Palette, Volume2, VolumeX,
} from "lucide-react";

const ICONS: Record<AppId, React.ComponentType<{ className?: string }>> = {
  terminal: TerminalSquare, monitor: Activity, password: KeyRound, network: Network,
  api: Globe, lab: FlaskConical, learn: GraduationCap, challenges: Gamepad2,
  wifi: Wifi, achievements: Trophy, vpn: Shield, settings: SettingsIcon, about: Info,
};

const DESKTOP_APPS: AppId[] = ["terminal", "monitor", "password", "network", "wifi", "vpn", "api", "lab", "learn", "challenges", "achievements"];

function renderApp(appId: AppId) {
  switch (appId) {
    case "terminal": return <Terminal />;
    case "monitor": return <SystemMonitor />;
    case "password": return <PasswordAnalyzer />;
    case "network": return <NetworkVisualizer />;
    case "api": return <ApiInspector />;
    case "lab": return <CyberLab />;
...
export function Desktop() {
  const { windows, open, theme, scanlines, setScanlines, matrixBg, setMatrixBg, sound, setSound, wallpaper, setWallpaper, iconPositions, setIconPosition } = useOS();
  ...
  useEffect(() => {
    document.documentElement.classList.remove("theme-purple", "theme-red", "theme-amber", "theme-ice");
  ...
  return (
    <div
      ...
      onClick={() => { setMenuOpen(false); setCtx(null); setPersonalize(false); }}
    ...
    >
      ...
      {DESKTOP_APPS.map((id, i) => {
        ...
        return (
          <button
            key={id}
            onMouseDown={(e) => { e.stopPropagation(); startDragIcon(id, e); }}
            onDoubleClick={() => { open(id); sfx.open(); }}
      ...
      <div className="absolute top-4 right-6 z-10 text-right text-xs font-mono space-y-1 opacity-80 flicker pointer-events-none">
        <span className="glitch-text neon-text text-sm tracking-widest" data-text="GEEK//OS">GEEK//OS</span>
      ...
      {/* Windows */}
      {windows.map((w) => (
        <Window key={w.id} win={w}>{renderApp(w.appId)}</Window>
      ))}

      {/* Context menu */}
      {ctx && (
        <div className="fixed z-[9000] glass-strong neon-border rounded-md py-1 text-xs min-w-44"
          style={{ left: Math.min(ctx.x, window.innerWidth - 200), top: Math.min(ctx.y, window.innerHeight - 200) }}
          onClick={(e) => e.stopPropagation()}>
          {[
            ...
            <button key={m.label}
              onClick={() => { m.action(); sfx.click(); setCtx(null); }}
      ...
      {/* Start menu */}
      {menuOpen && (
        <div className="fixed bottom-14 left-3 right-3 sm:right-auto z-[8500] glass-strong neon-border rounded-lg p-3 sm:w-72"
          onClick={(e) => e.stopPropagation()}>
          <div className="flex items-center gap-2 mb-3 text-xs opacity-70">
            <Search className="h-3 w-3" />
            <input value={search} onChange={(e) => setSearch(e.target.value)}
              placeholder="Search apps..." className="bg-transparent outline-none flex-1 text-xs" />
          </div>
          <div className="grid grid-cols-3 gap-2">
            {filtered.map((id) => {
              ...
              return (
                <button key={id}
                  onClick={() => { open(id); setMenuOpen(false); sfx.open(); }}
          ...
          <div className="border-t border-[var(--neon)]/20 mt-3 pt-2 flex justify-between text-xs">
            ...
            <button className="flex items-center gap-1 opacity-70 hover:text-[var(--neon)]" onClick={() => location.reload()}>
      ...
      {/* Taskbar */}
      <div className="fixed bottom-2 left-2 right-2 h-12 z-[8000] glass-strong neon-border rounded-xl flex items-center px-2 gap-2"
        onClick={(e) => e.stopPropagation()}>
        <button onClick={() => { setMenuOpen((v) => !v); sfx.click(); }}
        ...
        <div className="flex items-center gap-1 overflow-x-auto flex-1">
          {windows.map((w) => {
            ...
            return (
              <button key={w.id} onClick={() => useOS.getState().focus(w.id)}
        ...
        <div className="flex items-center gap-1 px-1">
          <button title="Personalize" onClick={() => { setPersonalize((v) => !v); sfx.click(); }}
            className="h-8 w-8 grid place-items-center rounded-md hover:bg-[var(--neon)]/15">
            <Palette className="h-4 w-4 text-[var(--neon)]" />
          </button>
          <button title={sound ? "Mute" : "Unmute"} onClick={() => setSound(!sound)}
            className="h-8 w-8 grid place-items-center rounded-md hover:bg-[var(--neon)]/15">
            {sound ? <Volume2 className="h-4 w-4 text-[var(--neon)]" /> : <VolumeX className="h-4 w-4 opacity-60" />}
          </button>
          <button title="VPN" onClick={() => { open("vpn"); sfx.open(); }}
            className="h-8 w-8 grid place-items-center rounded-md hover:bg-[var(--neon)]/15">
            <Shield className="h-4 w-4 text-[var(--neon)]" />
          </button>
          <button title="Settings" onClick={() => { open("settings"); sfx.open(); }}
            className="h-8 w-8 grid place-items-center rounded-md hover:bg-[var(--neon)]/15">
            <SettingsIcon className="h-4 w-4 text-[var(--neon)]" />
          </button>
        </div>
        <div className="text-xs opacity-90 px-2 flex items-center gap-2 sm:gap-3">
          <span className={online ? "text-[var(--neon)]" : "text-red-400"} title="Network">{online ? "●" : "✕"}</span>
      ...
      {/* Personalize panel */}
      {personalize && (
        <div className="fixed bottom-16 right-3 z-[8600] glass-strong neon-border rounded-lg p-3 w-72 space-y-3"
          onClick={(e) => e.stopPropagation()}>
          <div className="flex items-center gap-2 neon-text text-xs"><Palette className="h-3.5 w-3.5" /> Personalize</div>
          <div>
            <div className="text-[10px] opacity-60 mb-1">Wallpaper</div>
            <div className="grid grid-cols-3 gap-1.5">
              {(Object.keys(WALLPAPERS) as (keyof typeof WALLPAPERS)[]).map((w) => (
                <button key={w} onClick={() => { setWallpaper(w); sfx.click(); }}
          ...
          <button onClick={() => { open("settings"); setPersonalize(false); }}
...
function Quick({ label, value, onChange }: { label: string; value: boolean; onChange: (v: boolean) => void }) {
  return (
    <button onClick={() => onChange(!value)}
      className="w-full flex justify-between items-center px-2 py-1 text-xs hover:bg-[var(--neon)]/10 rounded">
      <span>{label}</span>
      <span className={`h-3.5 w-7 rounded-full relative ${value ? "bg-[var(--neon)]/60" : "bg-[var(--neon)]/15"}`}>
        <span className="absolute top-0.5 h-2.5 w-2.5 rounded-full bg-[var(--neon)] transition-all"
